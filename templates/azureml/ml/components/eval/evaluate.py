...
# =========================
X_test  = pd.read_parquet(os.path.join(args.X_test, "X_test.parquet"))
y_test  = pd.read_parquet(os.path.join(args.y_test, "y_test.parquet")).squeeze()

model   = joblib.load(os.path.join(args.model_output, "model.joblib"))
...
print(f"Metrics saved → {args.eval_output}")
